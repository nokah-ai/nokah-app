...
nokah_app.py
────────────
nokah — BIM Quality Intelligence
Interface client-facing (Alpha)

Usage : streamlit run nokah_app.py
"""
...
from bim_engine import (
    analyze_ifc, make_3d_figure, score_label,
    default_config, CONFIG_FILE,
    update_dataset, load_dataset,
    discipline_badge, scope_badge, scope_color,
    build_bim_json,
    run_anomaly_detection, generate_summary
)
...
# Client bubble max-width:60% of 760 = 456px, right-aligned in chat zone
# On full page: right edge ≈ 75-76% from left → gap ratio ≈ 3 parts gap, 1 part btn
_gap, _btn_col = st.columns([0.9, 1])
with _btn_col:
    if st.button(_t("Upload another IFC ↗","Analyser une autre maquette ↗"), use_container_width=False):
        st.session_state.nk_done = False
        st.session_state.nk_file = None
        st.rerun()
