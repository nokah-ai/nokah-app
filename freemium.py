...
# ── Configuration ──────────────────────────────────────────────────────────────
FREE_ANALYSES = 3          # Nombre d'analyses gratuites
STARTER_BONUS = 3          # Analyses bonus avec code STARTER
APP_URL = "https://nokah-app-8ecvxvmgro2m7jv6aszjjs.streamlit.app"
STRIPE_STARTER = "https://buy.stripe.com/3cIeV79JHbvwaj56Qab7y00"
STRIPE_PRO     = "https://buy.stripe.com/3cIeV79JHbvwaj56Qab7y00"  # à remplacer par lien Pro

# Codes d'accès valides (hashés pour sécurité)
ACCESS_CODES = {
    "NK-STARTER": {"plan": "starter", "extra_analyses": STARTER_BONUS},
    "NK-PRO":     {"plan": "pro",     "extra_analyses": 999999},
    # Ajouter ici les codes personnalisés pour prospects
...
def init_session():
    """Initialise les variables de session freemium."""
...
def can_analyze():
    """Retourne True si l'utilisateur peut encore analyser."""
    return get_remaining() > 0 or st.session_state.plan == "pro"


def increment_counter():
    """Incrémente le compteur après une analyse."""
    st.session_state.analyses_count += 1


def apply_access_code(code: str) -> bool:
    """Applique un code d'accès. Retourne True si valide."""
...
def render_analysis_counter():
    """Affiche le compteur d'analyses restantes en haut de l'app."""
    ...
    if plan == "pro":
        ...
        <div style="
            ...
            font-weight: 500;
        ">
            ⚡ Starter — {remaining} {'analysis' if remaining == 1 else 'analyses'} remaining
        </div>
        """, unsafe_allow_html=True)
    else:
        ...
        <div style="
            ...
            font-weight: 500;
        ">
            {'✓' if remaining > 0 else '✗'} Free — {remaining} {'analysis' if remaining == 1 else 'analyses'} remaining
        </div>
        """, unsafe_allow_html=True)


def render_paywall():
    """Affiche la page de blocage quand les analyses gratuites sont épuisées."""
    ...
    with col2:
        ...
        """, unsafe_allow_html=True)
        st.link_button("Start Pro →", STRIPE_PRO, use_container_width=True,
                       type="primary")

    # Séparateur
    st.markdown("<div style='text-align:center; color:#475569; margin: 24px 0; font-size:14px;'>— or enter an access code —</div>", unsafe_allow_html=True)
    ...
    st.markdown("<div style='text-align:center; margin-top:32px;'><a href='mailto:contact@nokah.ai' style='color:#475569; font-size:13px;'>Need a custom plan? Contact us →</a></div>", unsafe_allow_html=True)
